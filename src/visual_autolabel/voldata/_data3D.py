...
"""The subpackage of visual-autolabel that manages the 3D HCP data.

In order for the data in the volcnn package to work, you must have configured
...
# This value is initially set here, but it is then imported by the core volcnn
# package, and once this happens we will prefer the value found in volcnn to the
# value here (hence this one is called data_cache_path_init).

dataset_cache_path_init = os.environ.get('DATASET3D_CACHE_PATH')
if dataset_cache_path_init:
    dataset_cache_path_init = Path(dataset_cache_path_init)
...
default_zoom_init = os.environ.get('DATASET3D_ZOOM')
try:
    default_zoom_init = float(default_zoom_init)
except Exception:
    default_zoom_init = 0.5

default_subindex_init = (slice(2,-2), slice(8, 256+8), slice(2,-2))
...
def uncache_data(sid, name, fn,
    ...
    if cache_path is None:
        from . import dataset_cache_path as cache_path
    if cache_path is None:
...
def subdwi(sid, which, path=None, order=0):
    ...
    if path is None:
        ...
        from volcnn import noddi_data_path as path
    path = Path(path)
    path = path / str(sid)
    # What file?
...
def load_subject_data(sid, 
    ...
    if cache_path is Ellipsis:
        from . import dataset_cache_path as cache_path
    if subindex is Ellipsis:
        from . import default_subindex as subindex
    if zoom is Ellipsis:
        from . import default_zoom as zoom
    kwargs = dict(
...
def affine_correction(subindex=Ellipsis,
                      zoom=Ellipsis,
                      dtype=None,
                      device=None):
    if subindex is Ellipsis:
    ...
    if zoom is Ellipsis:
        from . import default_zoom as zoom
    affine = torch.eye(4, device=device, dtype=float)
    if subindex is not None:
        ii = (slice(0,None),) + tuple(subindex)
        for (ii,sl) in enumerate(subindex):
            affine[ii, -1] = sl.start
    if zoom is not None and zoom > 0:
        for ii in (0,1,2):
            affine[ii,ii] = 1/zoom
    return affine
def load_subject_affine(sid, 
                        dtype=None,
                        device=None,
                        subindex=None,
                        zoom=None,
                        forget=True):
    """Loads and returns an affine matrix for the given subject.

    The affine matrix returned by this function is appropriate for the images
    returned by the `load_subject_data` function. These images are cropped and
    downsampled, and the affine returned by this function respects that set of
    transformations.
    """
    subs = ny.data['hcp_lines'].subjects
    sub = subs[sid]
    subaff = torch.as_tensor(sub.images['brain'].affine)
    if forget:
        from pyrsistent import pmap
        object.__setattr__(subs, '_memoized', pmap())
        ny.hcp.forget_all()
    if zoom is None and subindex is None:
        return subaff
    else:
        if zoom is None:
            zoom = 1
        if subindex is None:
            subindex = (slice(0,None), slice(0,None), slice(0,None))
        aff0 = affine_correction(
            subindex=subindex,
            zoom=zoom,
            dtype=dtype,
            device=device)
...
class HCPVolumeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.data = pimms.lazy_map(
            {sid: ny.util.curry(lambda sid:load_subject_data(sid, **kw), sid)
             for sid in sids})
        affkw = dict(zoom=zoom, subindex=subindex, device=device, dtype=dtype) 
        self.affines = pimms.lazy_map(
            {sid: ny.util.curry(lambda sid:load_subject_affine(sid, **affkw), sid)
             for sid in sids})
    ...
    def get_affine(self, k, *, sid=None, index=None):
        if sid is None:
            if index is None:
                raise ValueError("sid or index is required")
            else:
                sid = self.sids[index]
        elif index is not None:
            raise ValueError("both sid and index cannot be given together")
        return self.affines[sid]
