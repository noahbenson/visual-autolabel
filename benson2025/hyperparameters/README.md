...
weights and model training data from the grid search were not retained.
  ...
  Azure-config
      visual-autolable_go.sh
         visual-autolabel_init
         docker run
  ...
  Azure
      build-gridsearch ?
         config.env
               GRIDSEARCH_DOCKERNAME
               GRIDSEARCH_VERS
               GRIDSEARCH_DOCKERTAG
               GRIDSEARCH_FILENAME
         docker build
         docker.save
      run-grid.sh ?
         config.env
               ...
               GRIDSEARCH_FILENAME
         docker_init
         key="`printf 'grid%05d' $ii`"
         docker_run key opts.json plan.json
         reads config.env
  Docker
      docker-compose.yml - running multiple containers
      Dockerfile - construct
         docker-run-grid.sh
             runs grid search for a single node
             $1 - starting grid-search cell
             $2 - stride of the search
             $3 - number of cells in the search
             gen-gridparams.py <(cat grid-params.json) $ii
                gen opts and plan
             train.py
