# -*- coding: utf-8 -*-
################################################################################
# visual_autolabel/image/_nyu.py
...
class NYURetinotopyImageCache(BilateralFlatmapImageCache):
    ...
    def fill_image(self, target, feature, im):
        ...
        im[torch.isnan(im)] = 0
        return im
NYURetinotopyImageCache.subjects = pimms.lazy_map(
    {s: ny.util.curry(NYURetinotopyImageCache.load_subject, s)
     for s in NYURetinotopyImageCache.subject_list})

class NYURetinotopyDataset(ImageCacheDataset):
    """A PyTorch Dataset object that encapsulates the NYU retinotopy dataset.

    The `NYURetinotopyDataset` is a PyTorch dataset for use with image-based
    models such as CNNs. The dataset may be configured to use any of a number of
    known features, including features based on the hand-drawn annotations. For
    a full list of possible features, check the `NYURetinotopyImageCache` type
    and the results of the `NYURetinotopyImageCache.builtin_features()` method.
    """
    __slots__ = ()
    def __init__(self, inputs, outputs,
        ...
        imcache = NYURetinotopyImageCache(
            hemis=hemis,
        ...
        sids = NYURetinotopyImageCache.subject_list
        if subjects is Ellipsis:
            subjects = sids
        # Make the targets dicts
        targets = tuple({'subject': s} for s in subjects)
...
def make_datasets(in_features, out_features,
    ...
    """
    if subjects is None or subjects is Ellipsis:
        subjects = NYURetinotopyImageCache.subject_list
    (trn_sids, val_sids) = make_partition(subjects, how=partition)
    def curry_fn(sids):
        return (lambda:NYURetinotopyDataset(
...
def make_dataloaders(in_features, out_features,
    ...
    subjects : list-like, optional
        ...
        `visual_autolabel.image.NYURetinotopyImageCache.subject_list`.
    ...
    """
    if exclusions is not None and exclusions is not Ellipsis:
        raise ValueError("exclusions must be None for nyu make_dataloaders")
    if raters is not None and raters is not Ellipsis:
        raise ValueError(
            "make_dataloaders() for NYU dataset does not accept raters")
